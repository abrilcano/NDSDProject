...
public class AirActor extends AbstractPersistentActor {
    ...
    public AirActor(int windowSize,int windowSlide, KafkaProducer<String, String> producer) {
    ...
    public static Props props(int windowSize, int windowSlide, KafkaProducer<String, String> producer) {
        return Props.create(AirActor.class, () -> new AirActor(windowSize, windowSlide, producer));
    }

    @Override
    public String persistenceId() {
        return "air-actor-" + getSelf().path().name();
    }
    ...
    private void sendToKafka(double value) {
        try{
            ...
            String message = String.format("{ %s Air quiality: %f}", aggregationType, value);
