...
public class HumActor extends AbstractPersistentActor {
    ...
    public static Props props(int windowSize, int windowSlide, KafkaProducer<String, String> producer) {
        return Props.create(HumActor.class, () -> new HumActor(windowSize, windowSlide, producer));
    }

    @Override
    public String persistenceId() {
        return "hum-actor-" + getSelf().path().name();
    }
    ...
    private void sendToKafka(double value) {
        try{
            ...
            String message = String.format("{ %s humidity: %f}", aggregationType, value);
