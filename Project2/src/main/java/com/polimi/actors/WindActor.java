...
public class WindActor extends AbstractPersistentActor {
    ...
    public WindActor(int windowSize,int windowSlide, KafkaProducer<String, String> producer) {
    ...
    public static Props props(int windowSize, int windowSlide, KafkaProducer<String, String> producer) {
        return Props.create(WindActor.class, () -> new WindActor(windowSize, windowSlide, producer));
    }

    @Override
    public String persistenceId() {
        return "wind-actor-" + getSelf().path().name();
    }
    ...
    private void sendToKafka(double value) {
        try{
            ...
            String message = String.format("{ %s wind speed: %f}", aggregationType, value);
